...
export default function Markdown({ logout }) {
  ...
  return (
    <DataContext.Provider value={{ setDataFromChild }}>
      <head>
        <title>StrikerX - Markdown Course</title>
      ...
      <div className="w-screen h-fit bg-white pt-[20px] pb-[20px] relative top-[-18px]">
          <HeaderUser logoutHead={logout} markdown={true} setDataFromChild={setDataFromChild}/>
          <p className='text-black ml-[20px] font-bold inter-txwe text-[35px]'>
            {dataFromChild ? 'ไวยากรณ์ Markdown' : 'Markdown Syntax'}
          </p>
          <div className='center w-fit ml-[20px] gap-[5px] text-[14px] mt-[-3px]'>
            <p className='text-black inter-txwe text-slate-600'>{dataFromChild ? 'อัปเดทล่าสุด: 19.07 โดย' : 'Last Update:  7.07 PM by'}</p>
            <Link to='/profile/mytxwedevs'>MyTxweDEVs</Link>
          </div>
          <div className='bg-black mx-[20px] h-[1px] mt-[10px]'></div>
          <p className='text-black font-bold text-[20px] inter-txwe ml-[20px] mt-[15px]'>{dataFromChild ? 'การแสดงข้อความเบื้องต้น' : 'Simple Text'}</p>
